...
const amountContent = document.querySelector(".amount");
const amountfather = document.querySelector(".amountContainer");
...
let provider, defaultProvider, signer;

function disbleButtons(boolean) {
...
let amountNfts = Number(amountContent.innerHTML);

amountfather.addEventListener('click', (element) => {
  if(element.target.className == 'add' && amountNfts < 3){
    amountNfts++;
    amountContent.innerHTML = amountNfts;
  }
  if(element.target.className == 'subtract' && amountNfts > 1){
    amountNfts--;
    amountContent.innerHTML = amountNfts;
  }
})
...
async function mint() {
  ...
  return await contract
    .mintLifeOutGenesis(amountNfts, { value: total })
    .then((tx) => {
...
async function getNftsSold() {
  const defaultProvider = ethers.getDefaultProvider();
  const contract = new ethers.Contract(contractAdress, contractAbi, defaultProvider);
...
btnRefresh.onclick = getNftsSold;
